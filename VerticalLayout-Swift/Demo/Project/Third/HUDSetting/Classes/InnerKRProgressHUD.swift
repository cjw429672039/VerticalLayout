...
private let hudViewMargin = VLScaleHeight(50)
private let hudViewPadding = CGFloat(15)
private let iconViewSize = CGSize(width: VLScaleHeight(50), height: VLScaleHeight(50))
...
extension KRProgressHUD {
    ...
    func updateLayouts(message: String?, iconType: KRProgressHUDIconType?, image: UIImage?, imageSize: CGSize?, isOnlyText: Bool) {
        ...
        case (nil, nil):
            ...
            DispatchQueue.main.async {//添加阴影
                self.iconView.superview!.addShadow(color: UIColor.black.withAlphaComponent(0.04), y: 4)
            }
        case (nil, let image):
...
extension UIView {
    //设置阴影
    func addShadow(color: UIColor? = UIColor.black.withAlphaComponent(0.02),
                   alpha: CGFloat = 1,
                   x width: CGFloat = 0,
                   y height: CGFloat = 4,
                   blur: CGFloat = 12,
